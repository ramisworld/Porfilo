 ...
 * Changes: brand-default color ramp, DPR cap, prefers-reduced-motion static
 * frame, and pause-when-hidden for battery/perf.
 */
...
interface AuroraProps {
  /** Left → mid → right gradient stops. Defaults to Porfilo indigo→violet→blue. */
  colorStops?: string[];
  amplitude?: number;
  blend?: number;
  speed?: number;
...
// Porfilo brand ramp: --color-accent (indigo) → --color-accent-2 (violet) → blue.
const BRAND_STOPS = ["#6c7bff", "#9a6cff", "#4080ff"];

export default function Aurora({
  colorStops = BRAND_STOPS,
  amplitude = 1.0,
  blend = 0.5,
  speed = 0.7,
  className = "",
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({
        ...
        antialias: true,
        // Cap DPR — the aurora is soft, so extra pixels buy nothing but heat.
        dpr: Math.min(typeof window !== "undefined" ? window.devicePixelRatio : 1, 1.5),
      });
    } catch {
      // No WebGL context available — degrade to the CSS layers behind us.
      return;
    }
